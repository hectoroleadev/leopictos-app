...
import { Play, Trash2, Check, X, Loader2, Edit2 } from 'lucide-react';
...
const PictogramCard: React.FC<PictogramCardProps> = ({ pictogram, onDelete, onEdit, isEditMode }) => {
  ...
  return (
    <div className={`relative group bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform ${!isEditing ? 'hover:-translate-y-1' : ''} overflow-hidden border-4 ${isEditing ? 'border-yellow-400' : 'border-blue-100 hover:border-blue-300 dark:border-gray-700 dark:hover:border-gray-600'}`}>
      ...
      <button 
        ...
        className={`w-full h-full flex flex-col items-center p-4 focus:outline-none focus:ring-4 focus:ring-blue-400 rounded-xl ${isPlaying ? 'cursor-wait' : 'cursor-pointer'}`}
      ...
      {isEditMode && !isDeleting && !isPlaying && !isSavingEdit && (
        <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity focus-within:opacity-100 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm p-1 rounded-full">
           {isEditing ? (
              <>
                <button 
